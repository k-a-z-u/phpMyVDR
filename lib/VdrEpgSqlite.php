<?php
	...
	class VdrEpgSqlite {
		...
		private function createIfNeeded() {
			...
			// create indexes
			$this->db->exec('CREATE INDEX IF NOT EXISTS idx_show_channel ON show (channelIdx);');
			$this->db->exec('CREATE INDEX IF NOT EXISTS idx_show_event_start ON show (eventTsStart);');
			$this->db->exec('CREATE INDEX IF NOT EXISTS idx_show_event_range ON show (eventTsStart, eventTsEnd);');
			//$this->db->exec('CREATE INDEX IF NOT EXISTS idx_show_event_end ON show (eventTsEnd);');
			//$this->db->exec('CREATE INDEX IF NOT EXISTS idx_show_event_time_2 ON show (eventTsEnd);');
		...
		public function addEpgEntry(VdrEpgEntry $entry) {
			...
			$eTsEnd = $entry->getEvent()->getTsEnd();
			...
			$this->stmtAddShow->bindParam(4, $eTsEnd, SQLITE3_INTEGER);
		...
		public function removeOldEpgEntries() {
			$ts = time();
			$this->db->exec("DELETE FROM show WHERE (eventTsEnd) < {$ts}");
			$this->db->exec('VACUUM');
		}
	...
	function getEpgEntryByRow($row) {
		...
		$event = new VdrEpgEvent($row['eventID'], $row['eventTsStart'], $row['eventTsEnd']);
		$genre = $row['genre'];
